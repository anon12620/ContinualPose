...
from mmpose.registry import DATASETS, EVALUATORS, TRANSFORMS


@EVALUATORS.register_module()
class UnifiedDatasetEvaluator(Evaluator):
    ...
    Args:
        ...
        converters (Optional[Sequence[dict]]): The configs of keypoint
            converters. Default: None.
    """
    ...
    def __init__(
        ...
        converters: Optional[Sequence[dict]] = None,
    ):
        ...
        if converters is None:
            converters = [None] * len(datasets)

        assert len(converters) == len(datasets), 'the argument ' \
            'datasets should have same length as converters'
        ...
        for dataset, converter, metric in zip(datasets, converters, self.metrics):
            ...
            if converter is not None:
                converters_dict[dataset_meta['dataset_name']
                                ] = TRANSFORMS.build(converter)
    ...
    def process(self,
        ...
        for dataset_name, metric in self.metrics_dict.items():
            if dataset_name in _data_samples:
                ...
                # Convert keypoints to target dataset
                converter = self.converters_dict.get(dataset_name, None)
                if converter is not None:
                    _converted_data_samples = []
                    for sample in _data_samples[dataset_name]:
                        sample['gt_instances'] = converter(
                            sample['gt_instances'])
                        sample['pred_instances'] = converter(
                            sample['pred_instances'])
                        _converted_data_samples.append(sample)
                else:
                    _converted_data_samples = _data_samples[dataset_name]

                metric.process(data_batch, _converted_data_samples)
            else:
                continue
