...
model = dict(  # initial model
    ...
    init_cfg=dict(
        type='Pretrained',
        checkpoint='work_dirs/rtmpose-t/task-1/rtmpose-t_8xb256-210e_crowdpose-256x192/best_crowdpose_AP_epoch_210.pth'),
...
# hooks
default_hooks = dict(
    checkpoint=dict(save_best='crowdpose/AP', rule='greater', max_keep_ckpts=1))
...
val_evaluators = [
    ...
    dict(type='UnifiedDatasetEvaluator',
         metrics=[crowdpose_metric],
         datasets=[dataset_crowdpose_val],
         converters=[None]),

    # experience 2 (model predicts 21 keypoints, validate on 16, 17 keypoints)
    dict(type='UnifiedDatasetEvaluator',
         metrics=[crowdpose_metric, coco_metric],
         datasets=[dataset_crowdpose_val, dataset_coco_val],
         converters=[None, None]),

    # experience 3 (model predicts 21 keypoints, validate on 14, 16, 17 keypoints)
    dict(type='UnifiedDatasetEvaluator',
         metrics=[crowdpose_metric, coco_metric, mpii_metric],
         datasets=[dataset_crowdpose_val, dataset_coco_val, dataset_mpii_val],
         converters=[None, None, mpii_converter]),
]
